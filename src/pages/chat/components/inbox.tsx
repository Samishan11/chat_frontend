        ...
        <div className=" bg-indigo-500 rounded-t-lg max-h-20 w-ful flex p-3 items-center">
          <div className="flex relative items-center justify-center h-8 w-8 bg-indigo-200 rounded-full">
            {auth._id === data?.requestBy?._id
              ...
              : data?.requestBy?.username.slice(0, 1).toUpperCase()}
          </div>
          <div className=" text-sm ml-1 font-semibold text-white">
            {" "}
            {auth._id === data?.requestBy?._id
              ? data?.requestTo?.username.toUpperCase()
              : data?.requestBy?.username.toUpperCase()}
          </div>
        </div>
        <div className="flex flex-col flex-auto flex-shrink-0 rounded-2xl bg-gray-100 h-[90%] p-4">
